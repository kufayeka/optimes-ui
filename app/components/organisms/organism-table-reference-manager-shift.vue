<template>
  <molecules-molecule-form-section title="Shift Manager">
    <template #endSection>
    ...
    <molecules-molecule-popup-edit title="Create Reference Data" max-width="700" :open="_state_popup_create_reference_data" @close="ui_command_popup_create_reference_data_close">
      <molecules-molecule-popup-content-base>
        <template #content>
          <atoms-atom-base-wrapper max-width="100%" max-height="400px">
            <molecules-molecule-form-create-generic debug :form-template="create_reference_form_template" :initial-data="create_reference_initial_data" v-model:formData="_data_created_reference_data"/>
          </atoms-atom-base-wrapper>
        </template>
        <template #actions>
...
const edit_reference_form_template = computed(() => [
  { key: 'key', label: 'Reference Key', type: 'text', required: true, disabled: true },
  { key: 'value.shift_name', label: 'Shift Name', type: 'text', required: true }, 
  { key: 'value.shift_start_time', label: 'Shift Start Time', type: 'time', required: true }, 
  { key: 'value.shift_end_time', label: 'Shift Finish Time', type: 'time', required: true }, 
...
const create_reference_initial_data = computed(() => {
  if (!_state_popup_create_reference_data.value) {
    return null; 
  } else {
    // pop up open
    return {
      key: `reference/work_shift/${uuidv4()}`,
      value: { 
        shift_name: '',
        shift_start_time: '',
        shift_end_time: '',
