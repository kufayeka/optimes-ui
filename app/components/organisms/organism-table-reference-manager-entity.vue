<template>
  <molecules-molecule-form-section title="Entity Manager">
    <template #endSection>
...
const edit_reference_form_template = computed(() => [
  ...
  { key: 'value.machine_name', label: 'Machine Name', type: 'text', required: true },
  { key: 'value.machine_model_number', label: 'Machine Model Number', type: 'text', required: true },
...
const create_reference_initial_data = computed(() => {
  if (!_state_popup_create_reference_data.value) {
    ...
    // pop up open
    return {
      key: `reference/entity/machine/${uuidv4()}`,
      value: { 
        machine_name: '',
